...
public class JiFenDetail extends Activity {

	private ListView mLvFenXiang;
	private JiFenDetailFenXiangAdapter mAdapter;
	private String url;
	private String url2;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Boolean isNetWork = netUtil.isNetworkAvalible(JiFenDetail.this);
		if (isNetWork) {
			loadDate();
		} else {
			Toast.makeText(JiFenDetail.this, "网络连接不可用", Toast.LENGTH_SHORT)
					.show();
		}
	}
	...
	private void initView() {
		...
		mAdapter = new JiFenDetailFenXiangAdapter(this);
						...
						new Listener<KaoPingJiFenModel>() {
							...
							public void onResponse(KaoPingJiFenModel arg0) {
								if (arg0!=null) {
									...
									ShiJiFenZhiDangNei.setText(DangNei + "");
									...
									DecimalFormat decimalFormat1 = new DecimalFormat(
											"0.00");
									String Shang1 = decimalFormat1
											.format(shang1);
									ShiJiFenZhiDangWai.setText(DangWai + "");
									...
									intent.putExtra("yzongfen", zongfen);
									setResult(1, intent);
									
									Preferences.getPrefer(getApplicationContext()).putString("yizongfen", zongfen);
							...
							public void onErrorResponse(VolleyError arg0) {
								Intent intent = getIntent();
								intent.putExtra("zongfen", Preferences.getPrefer(getApplicationContext()).getString("yizongfen", ""));
								setResult(0,intent);
								Toast.makeText(getApplicationContext(), "没有相关数据", Toast.LENGTH_SHORT).show();
							}
						}), url2);
